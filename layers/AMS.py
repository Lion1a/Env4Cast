...
class AMS(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
        ...
        # self.wavelet_layer = WaveletLayer(pred_len=0, k=3)
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        x1 = x[..., 0]  # 去掉最后一维
        # low, high = self.wavelet_layer(x1)
        #
        # contains_zero2 = torch.any(high == 0)
        # contains_zero = torch.any(low == 0)
        #
        # print("Contains zero:", contains_zero.item(),contains_zero2.item())  # .item() 用于获取布尔值
        ...
        # print("expert_inputs_len:",len(expert_inputs))
        # print("expert_inputs:",expert_inputs[0].shape,expert_inputs[1].shape)
        expert_outputs = [self.experts[i](expert_inputs[i])[0] for i in range(self.num_experts)]
        # print("expert_inputs_len:",expert_inputs[0].shape,expert_inputs[1].shape,expert_inputs[2].shape,expert_inputs[3].shape)
